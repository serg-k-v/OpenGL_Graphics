...
class GLWidget(QGLWidget):
    ...
    def draw(self):
        if self.text == 'GL_POINT':
            print('GL_POINT')
            prm.draw_point(10, [.1, .1, .5], [5, 5, 0], False)
        if self.text == 'GL_LINES':
            print('GL_LINES')
            prm.draw_line(5, [1., .5, 1.], [0, 0, 0], [500, 500, 0], False)
        ...
        if self.text == 'GL_LINE_LOOP':
            ...
            [290, 510, 0], [420, 390, 0], [190, 130, 0])
        if self.text == 'GL_TRIANGLES':
            # print('GL_TRIANGLES')
            prm.draw_triangle(15, [0.2, 0.3, 0.8], False, 'line')
        if self.text == 'GL_TRIANGLE_STRIP':
            # print('GL_TRIANGLE_STRIP')
            prm.draw_triangle_strip(10, [.5, 0.1, 0.6], [[420, 500, 0],
            [30, 510, 0], [70, 70, 0], [250, 250, 0]], False, 'line')
        if self.text == 'GL_TRIANGLE_FAN':
            # print('GL_TRIANGLE_FAN')
            prm.draw_triangle_fan(50, [.4, 0.2, 0.7], [[420, 500, 0],
            [30, 510, 0], [70, 70, 0], [250, 250, 0]], False, 'line')
